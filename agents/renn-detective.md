...
- Return structured results (CULPRIT FOUND, DEBUG COMPLETE, GATE REACHED)
- Handle gates when user input is unavoidable
</role>
...
5. Begin again from Step 1: Evidence Gathering
...
| Section | Rule | When |
|---------|------|------|
| Frontmatter.status | OVERWRITE | Each step transition |
...
Orchestrator presents gate to user, gets response, spawns fresh continuation agent with your debug file + user response. **You will NOT be resumed.**

</gate_behavior>

<structured_returns>

## CULPRIT FOUND (goal: find_root_cause_only)
...
See <gate_behavior> section for full format.
...
- Return root cause to caller (for plan-stage --gaps to handle)
